...
To achieve $O(\log(m \times n))$ time complexity, we perform a standard binary search on this "virtual" 1D array without
taking up the memory to actually flatten it.

### Core Concept: Index Mapping

We map the 1D index back to the 2D matrix coordinates on the fly. Let `n` be the number of columns in the matrix.
For any 1D index `i`:

* **Row Index:** `i / n` (Integer division gives the row)
* **Col Index:** `i % n` (Modulo operator gives the column)
