...
- It will be solved using prefix and Suffix array
- where you have to find the Maximum elmement to left and maximum element to the right.
- take the min(left,right)
- if min of both of them if greater than you bar height then you can store some water.
- which `unitsOfWater= min-height[curr]`

# Time Complexity O(N)

- as we are running loops `O(N)+O(N)+O(N)` = 3*`O(N)`
- so the Overall time Complexity `O(N)`

# Space Complexity O(N)

- as we are storing previous min and previous max so `O(N)`
